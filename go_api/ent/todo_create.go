...
func (tc *TodoCreate) SetAssaginPerson(s string) *TodoCreate {
	tc.mutation.SetAssaginPerson(s)
	return tc
}
...
func (tc *TodoCreate) SetIsComplete(b bool) *TodoCreate {
	tc.mutation.SetIsComplete(b)
	return tc
}

// SetNillableIsComplete sets the "is_complete" field if the given value is not nil.
func (tc *TodoCreate) SetNillableIsComplete(b *bool) *TodoCreate {
	if b != nil {
		tc.SetIsComplete(*b)
	}
	return tc
...
func (tc *TodoCreate) createSpec() (*Todo, *sqlgraph.CreateSpec) {
	...
	if value, ok := tc.mutation.AssaginPerson(); ok {
		_spec.SetField(todo.FieldAssaginPerson, field.TypeString, value)
		_node.AssaginPerson = value
	}
